...
export const arrayToEnum = <T extends readonly string[]>(arr: T): ArrayToEnum<T> => {
  return Object.fromEntries(arr.map((key) => [key, key])) as { readonly [K in T[number]]: K };
};

export const arrayToMirrorLowercase = <T extends readonly string[]>(arr: T): ArrayToMirrorLowerCase<T> => {
  return Object.fromEntries(arr.map((key: string) => [key, key.toLocaleLowerCase()])) as ArrayToMirrorLowerCase<T>;
};
